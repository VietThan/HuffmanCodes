...
void HuffmanCodes(char data[], int freq[], int size, std::ofstream &EncodeFile);
...
//    int freq[] = { 5, 9, 12, 13, 16, 45 };
    ...
    while(!InputFile.eof()){
        ...
        if (c != -1){
            ...
            auto itr = symbolFreq.find(c);

            if (itr == symbolFreq.end()){
                symbolFreq.insert({c, 1});
            } else {
                itr->second += 1;
            }
        }
 ...
 *
 * @param data      char[] (pointer), the symbols sorted by frequencies
 ...
 *
 * @param data      char[],                 the symbols sorted by frequencies
